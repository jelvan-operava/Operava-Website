...
import {
  ...
  getAssessment,
  listApplicantMessages,
  recruitmentConfigured,
  saveApplicantApplicationTxt,
  saveAssessment,
...
import {
  ...
  formatApplicantAssessmentText,
  recruitmentMegaFileName,
  type MegaBackupEnv,
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (done) {
      ...
      try {
        const rows = await listApplicantMessages(env, session.applicationId)
        messages = rows.map((m) => ({
          role: m.role,
          body: m.body,
        ...
        console.error('list messages failed', err instanceof Error ? err.message : 'unknown')
      }

      const additional = applicant?.additional || ''
      const resumeInfo =
        additional && /resume|cv/i.test(additional)
          ? additional
          : additional
      ...
      const record = {
        ...
        otpConfirmed: Boolean(applicant?.otp_confirmed ?? applicant?.email_verified ?? true),
        otpConfirmedAt: applicant?.email_verified_at || '',
        termsAccepted: Boolean(applicant?.terms_accepted ?? true),
        privacyAccepted: Boolean(applicant?.privacy_accepted ?? true),
        messages,
        resumeInfo,
        assessment: {
          correctCount,
      ...
      const fileName = recruitmentMegaFileName(session.applicationId, fullName)
      const applicationTxt = formatApplicantAssessmentText(record)

      try {
        await saveApplicantApplicationTxt(env, session.applicationId, fileName, applicationTxt)
      } catch (err) {
        console.error('save application txt failed', err instanceof Error ? err.message : 'unknown')
      }

      const megaTask = backupRecruitmentAssessmentToMega(env, record)
      ...
      return json({
        ...
        name: fullName,
        message: passed
          ? 'Assessment complete. You passed and are eligible for the recruitment pool. A confirmation email has been sent (Talent Acquisition is notified with the application file).'
