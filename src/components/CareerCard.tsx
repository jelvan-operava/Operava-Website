...
export default function CareerCard({
  ...
  /*
   * Portrait flip card — both faces share one fixed box (same size & position).
   * Front is in normal flow to set height; back is absolute inset-0 on top of it.
   * Inner rotator uses preserve-3d + rotateY so the front hides and back appears
   * like a physical picture card flipping in place.
   */
  return (
    <motion.div
    ...
    >
      <div
        className="relative w-full h-full transition-transform duration-600 ease-[cubic-bezier(0.4,0.0,0.2,1)]"
        style={{
          transformStyle: 'preserve-3d',
          transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)',
        }}
      >
        {/* FRONT face — establishes size; stays in place when flipped (hidden by backface) */}
        <div
          className="operava-card-frame group relative w-full h-full flex flex-col items-center justify-between"
          style={{
        ...
        {/* BACK face — exact same box (absolute inset-0), mirrored 180° */}
        <div
          className="operava-card-frame absolute inset-0 w-full h-full flex flex-col items-stretch justify-between overflow-hidden"
          style={{
            backfaceVisibility: 'hidden',
            WebkitBackfaceVisibility: 'hidden',
        ...
        >
          ...
          <div className="relative z-10 w-full h-full flex flex-col min-h-0 p-0">
