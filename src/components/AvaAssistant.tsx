...
const WELCOME =
  "Hello — I'm AVA.\n\nI explain OPERAVA services in plain language: what they mean, how delivery typically works, and practical examples — for technology, operations, workforce, and careers. I only use OPERAVA's controlled knowledge (not the open web), and I do not share prices, private client data, or internal staff records.\n\nAsk about a service, or say if you are exploring as a client or applying for a role. Formal quotes go through Request a Quote; applications go through Careers."
          ...
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-gray-50/80">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex gap-2.5 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                ...
                <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 ${msg.sender === 'user' ? 'rounded-tr-xs bg-violet-700 text-white' : 'rounded-tl-xs bg-white border border-gray-200/80 shadow-sm text-gray-800'}`}>
                  ...
                  <div className={`mt-1.5 flex items-center gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    {msg.sender === 'ava' && (
                      <button
                        ...
                        className="p-0.5 text-gray-400 hover:text-violet-600"
