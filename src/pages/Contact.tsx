...
import { contactChannels, verificationPortalUrl, verificationEmail } from '../data/contacts'
    ...
    <main className="bg-white pt-28 pb-20">
      <div className="max-w-3xl mx-auto px-6">
        ...
        <p className="text-sm text-gray-600 mb-8">
          ...
          <Link to="/contacts" className="text-violet-700 underline font-medium">
            Contacts
          </Link>
          . Document reference checks:{' '}
          <a href={verificationPortalUrl} className="text-violet-700 underline font-medium" target="_blank" rel="noopener noreferrer">
            Verification Portal
          </a>
          .
        </p>
        ...
        <section className="mt-14 pt-10 border-t border-gray-100">
          <h2 className="text-xl font-bold text-gray-900 mb-2">Department contacts</h2>
          <p className="text-sm text-gray-500 mb-6">
            Prefer email or WhatsApp for a specific team. Verification team:{' '}
            <a href={`mailto:${verificationEmail}`} className="text-violet-700 font-medium">
              {verificationEmail}
            </a>
            .
          </p>
          <ul className="space-y-4">
            {contactChannels.map((c) => (
              <li key={c.id} className="rounded-xl border border-gray-100 p-4">
                ...
                <div className="mt-2 flex flex-wrap gap-3">
                  {c.email && (
                    <a href={`mailto:${c.email}`} className="inline-flex items-center gap-1.5 text-sm text-violet-700 font-medium">
                  ...
                  {c.url && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 text-sm text-violet-700 font-medium"
                    >
                      {c.id === 'verification' ? <ShieldCheck className="w-3.5 h-3.5" /> : <ExternalLink className="w-3.5 h-3.5" />}
                      {c.url.replace(/^https?:\/\//, '')}
                    </a>
                  )}
                </div>
