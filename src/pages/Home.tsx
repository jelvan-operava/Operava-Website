  ...
  return (
    <>
      ...
      <main className={`overflow-x-hidden transition-opacity duration-700 ${isHomeReady ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="w-full max-w-[100rem] mx-auto px-5 sm:px-8 lg:px-12 xl:px-16 relative z-10">
            ...
            <div className="space-y-16 sm:space-y-20 lg:space-y-28">
              {[
                ...
                <div key={item.title} className={`reveal reveal-delay-${(i % 3) + 1} flex flex-col ${item.reverse ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-10 lg:gap-16 xl:gap-20`}>
                  <div className="w-full lg:w-1/2 flex flex-col justify-center">
                    <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 tracking-tight leading-tight mb-4">{item.title}</h3>
                    ...
                    <div>
                      <Link to={item.link} className="inline-flex items-center gap-2.5 px-6 py-3.5 rounded-xl bg-violet-700 text-white font-semibold text-sm hover:bg-violet-800 active:scale-95 transition-all duration-200 shadow-md shadow-violet-700/20 group/btn">
                        <span>{item.cta}</span>
                  ...
                  <div className="w-full lg:w-1/2 flex items-center justify-center">
                    <div className={`w-full flex items-center justify-center p-2 sm:p-6 bg-transparent ${item.floatClass}`}>
                      <img src={item.imageSrc} alt={item.title} className="w-full max-w-[340px] sm:max-w-[420px] lg:max-w-[460px] h-auto object-contain pointer-events-none select-none drop-shadow-xl transition-all duration-700 ease-out hover:scale-105" loading="lazy" referrerPolicy="no-referrer" />
                    </div>
                  </div>
                </div>
              ))}
              ...
              {operatingModelSteps.map((step, i) => (
                <div key={step.step} style={{ animationDelay: `${(i % 3) * 0.4}s` }} className={`reveal reveal-delay-${Math.min(i + 1, 5)} group relative bg-transparent flex items-center justify-center transition-all duration-500 hover:scale-[1.02] select-none`}>
                  <div className="relative w-full overflow-hidden bg-transparent">
                    <img src={step.imageSrc} alt={`${step.step} - ${step.label}: ${step.desc}`} className="w-full h-auto object-contain pointer-events-none select-none transition-transform duration-500 ease-out group-hover:scale-[1.02]" loading="lazy" referrerPolicy="no-referrer" />
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Full remaining sections (why carousel, workforce, tech, tools, signature) restored from main pre-hero-refactor */}
        ...
        <section ref={signatureRef} className="pt-0 pb-6 sm:pb-8 bg-[#FBFBFA] relative overflow-hidden">
          <div className="w-full max-w-[100rem] mx-auto px-5 sm:px-8 lg:px-12 xl:px-16 w-full flex justify-start items-center relative z-10">
            <div className="reveal flex justify-start items-center bg-transparent max-w-sm sm:max-w-md">
              <img src="https://res.cloudinary.com/sdaxzncs/image/upload/operava-signature.jpg" alt="OPERAVA Signature" className="w-full h-auto max-h-[160px] sm:max-h-[200px] object-contain object-left pointer-events-none select-none block mix-blend-multiply transition-transform duration-500 hover:scale-[1.01]" loading="lazy" referrerPolicy="no-referrer" />
            </div>
          </div>
        </section>
      </main>
