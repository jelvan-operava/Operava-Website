  ...
  return (
    <>
      ...
      <main className={`overflow-x-hidden transition-opacity duration-700 ${isHomeReady ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <section ref={whyRef} className="relative bg-gray-950 text-white overflow-hidden">
          <div className="max-w-7xl mx-auto px-6 lg:px-8 pt-16 lg:pt-20 pb-6 relative z-10">
            <h2 className="reveal text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight leading-tight text-white">
              {t('why.title', 'Engineered for Performance & Scalability')}
            </h2>
          </div>

          <div className="relative w-full min-h-[70vh] sm:min-h-[75vh] lg:min-h-[85vh] flex items-center justify-center">
            {whyFeaturesList.map((f, i) => (
              <div
                key={f.title}
                className={`absolute inset-0 flex items-center justify-center px-4 sm:px-8 lg:px-12 transition-all duration-500 ease-out ${
                  i === whySlide
                ...
                aria-hidden={i !== whySlide}
              >
                <img
                  src={f.imageSrc}
                  alt={f.title}
                  className="w-full h-full max-h-[70vh] sm:max-h-[75vh] lg:max-h-[82vh] object-contain select-none pointer-events-none"
            ...
            <div className="absolute inset-x-0 bottom-6 sm:bottom-10 z-20 flex items-center justify-center gap-4 sm:gap-6">
              <button
                type="button"
                aria-label="Previous"
                onClick={() => setWhySlide((s) => (s - 1 + whyFeaturesList.length) % whyFeaturesList.length)}
                className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 backdrop-blur-md flex items-center justify-center transition-colors"
              >
                ...
                {whyFeaturesList.map((_, i) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all duration-300 ${
                      i === whySlide ? 'w-8 bg-violet-400' : 'w-2 bg-white/40 hover:bg-white/60'
                    }`}
                  />
                ))}
              </div>
